...
class EdgarClient:
    ...
    def list_target_filings(self, cik: str) -> list[FilingMeta]:
        """Latest annual report (10-K or 20-F), recent quarterly/interim reports
        (10-Q or 6-K), and current reports (8-K or 6-K) from the lookback window.

        Supports both US domestic filers (10-K/10-Q/8-K) and foreign private
        issuers (20-F/6-K) so ADRs and cross-listed companies are indexed too.
        """
        ...
        # Annual report: 10-K (US domestic) preferred; fallback to 20-F (foreign private issuer)
        ten_ks = [f for f in all_filings if f.form == "10-K"][:1]
        twenty_fs = [f for f in all_filings if f.form in ("20-F", "20-F/A")][:1]
        annual = ten_ks if ten_ks else twenty_fs

        # Quarterly/interim: 10-Q (US) preferred; fallback to 6-K (foreign)
        ten_qs = [f for f in all_filings if f.form == "10-Q"][: settings.filings_num_10q]
        six_ks_all = [f for f in all_filings if f.form == "6-K" and f.filing_date >= cutoff]
        quarterly = ten_qs if ten_qs else six_ks_all[: settings.filings_num_10q]

        # Current/event reports: 8-K (US) preferred; supplement with 6-K
        eight_ks = [f for f in all_filings if f.form == "8-K" and f.filing_date >= cutoff]
        # For foreign filers, 6-Ks that weren't used as quarterly reports serve as event reports
        used_six_k_ids = {f.accession for f in quarterly}
        event_six_ks = [f for f in six_ks_all if f.accession not in used_six_k_ids]
        event_reports = eight_ks + event_six_ks

        selected = annual + quarterly + event_reports
        log.info(
            "filings_selected",
            cik=cik,
            annual_form=annual[0].form if annual else "none",
            n_quarterly=len(quarterly),
            n_events=len(event_reports),
            total=len(selected),
        )
        return selected
